...
- В `.env` задан `FRONTEND_URL=http://192.168.31.130:12000`.
- В `CORS_EXTRA_ORIGINS` должен быть тот же origin, с которого открываете сайт (например `http://192.168.31.130:12000`), иначе бэкенд может отклонить запросы при прямом обращении.
