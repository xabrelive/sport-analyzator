...
async def main():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        data3 = r3.json() if r3.is_success else None
        print(f"Status: {r3.status_code}")
        if r3.is_success and data3:
        ...
        # ---- 4) GET /v4/bet365/prematch?FI=bet365_id (bet365_id из view п.3) ----
        print("=" * 60)
        bet365_id = None
        if r3.is_success and data3:
            res3 = data3.get("results")
            if isinstance(res3, dict) and res3:
                first_ev = next(iter(res3.values()), None)
                if isinstance(first_ev, dict):
                    bet365_id = first_ev.get("bet365_id")
            elif isinstance(res3, list) and res3 and isinstance(res3[0], dict):
                bet365_id = res3[0].get("bet365_id")
        if not bet365_id:
            print("4) GET /v4/bet365/prematch — пропуск (нет bet365_id из view)")
        else:
            print(f"4) GET /v4/bet365/prematch?FI={bet365_id}")
            print("=" * 60)
            r4 = await client.get(
                f"{BASE_V4}/bet365/prematch",
                params={"token": token, "FI": str(bet365_id)},
            )
            print(f"Status: {r4.status_code}")
            if r4.is_success:
                data4 = r4.json()
                print("Верхний уровень:", list(data4.keys()))
                res4 = data4.get("results") or data4
                if isinstance(res4, dict):
                    markets = res4.get("markets")
                    if isinstance(markets, list):
                        print(f"results.markets: list, len={len(markets)}")
                        for i, m in enumerate(markets[:5]):
                            if isinstance(m, dict):
                                print(f"  [{i}] name/id={m.get('name') or m.get('id')!r}, outcomes: {len(m.get('outcomes') or m.get('choices') or [])}")
                    else:
