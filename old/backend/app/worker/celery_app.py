    ...
    beat_schedule.update(
        {
            ...
            # Лёгкий фон для линии: догрузка коэффициентов по матчам без line OddsSnapshot (scheduled/pending_odds).
            "backfill-line-odds": {
                "task": "app.worker.tasks.collect_betsapi.backfill_line_odds",
                "schedule": settings.line_odds_backfill_interval_seconds,
                "options": {"queue": "betsapi_collect", "expires": 300},
            },
            # Догрузка результатов матчей (MatchResult) — нужна для корректной статистики исходов.
            # Используем интервал и batch_size из конфига (по умолчанию раз в 2 минуты).
            "backfill-missing-results": {
                "task": "app.worker.tasks.collect_betsapi.backfill_missing_results",
                "schedule": settings.result_backfill_interval_seconds,
