...
function formatLeadTime(seconds: number | null | undefined): string {
  ...
  if (hours > 0) return `${sign}${hours} ч ${remMin} мин`;
...
export default function TableTennisLineNnPage() {
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold text-white mb-2">Настольный теннис — аналитика нейросетью</h1>
        <p className="text-slate-400 text-sm">Прематч‑прогнозы отдельной нейросетевой модели по архивным данным.</p>
      </div>

      {locked && (
        <div className="rounded-lg border border-amber-500/60 bg-amber-900/20 px-4 py-3 text-sm text-amber-100 space-y-1">
          <p className="font-medium">Доступ к аналитике нейросетью недоступен.</p>
          <p className="text-amber-100/90">
            {lockedMessage || "Чтобы видеть прогнозы нейросети по предстоящим матчам, оформите подписку на аналитику."}
          </p>
          <div className="pt-1">
            <Link
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="inline-flex items-center gap-1">
          ...
          <button type="button" onClick={() => setPhaseFilter("all")} className={`px-2 py-1 rounded-md text-xs ${phaseFilter === "all" ? "bg-sky-500/20 border border-sky-500/40 text-sky-100" : "border border-slate-700 text-slate-400 hover:text-slate-200"}`}>Все</button>
          <button type="button" onClick={() => setPhaseFilter("upcoming")} className={`px-2 py-1 rounded-md text-xs ${phaseFilter === "upcoming" ? "bg-sky-500/20 border border-sky-500/40 text-sky-100" : "border border-slate-700 text-slate-400 hover:text-slate-200"}`}>Ещё не начались</button>
          <button type="button" onClick={() => setPhaseFilter("live")} className={`px-2 py-1 rounded-md text-xs ${phaseFilter === "live" ? "bg-sky-500/20 border border-sky-500/40 text-sky-100" : "border border-slate-700 text-slate-400 hover:text-slate-200"}`}>Уже идут</button>
        </div>
            ...
            <table className="w-full text-xs md:text-sm">
              <thead>
                <tr className="border-b border-slate-700 bg-slate-800/80 text-slate-300">
                  ...
                  <th className="px-3 py-2 text-left font-medium min-w-[220px]">Прогноз (NN)</th>
              ...
              <tbody>
                {filteredItems.map((f, idx) => (
                  <tr key={`${f.id ?? f.event_id}-${f.market ?? "match"}-${f.channel ?? "nn"}`} className={`border-b border-slate-700/60 hover:bg-slate-700/25 transition ${idx % 2 === 0 ? "bg-slate-900/10" : "bg-transparent"}`}>
                    ...
                    <td className="px-3 py-2 text-center text-slate-300 tabular-nums whitespace-nowrap">{formatLeadTime(f.forecast_lead_seconds ?? null)}</td>
                    <td className="px-3 py-2 text-white">
                      <Link href={`/dashboard/table-tennis/matches/${encodeURIComponent(f.event_id)}`} className="text-emerald-300 hover:text-emerald-200 font-semibold">
                        {f.home_name} — {f.away_name}
                      </Link>
                    </td>
                    <td className="px-3 py-2 text-slate-400 whitespace-nowrap">
                      <span className="inline-flex items-center rounded-md border border-slate-600 bg-slate-800/60 px-2 py-1 text-xs">{f.league_name ?? "—"}</span>
                    </td>
                    <td className="px-3 py-2 text-slate-100">{f.forecast_text || "—"}</td>
                    <td className="px-3 py-2 text-center text-slate-200 tabular-nums whitespace-nowrap">{f.forecast_odds != null ? f.forecast_odds.toFixed(2) : f.odds_used != null ? f.odds_used.toFixed(2) : "—"}</td>
                    <td className="px-3 py-2 text-center">
                      <span className={`text-xs font-medium ${statusTextClass(f)}`}>{formatStatus(f)}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {!loading && !error && (
        <div className="mt-3 flex items-center gap-2">
          <button className="rounded-md border border-slate-700 px-3 py-1.5 text-slate-300 disabled:opacity-50" disabled={page <= 1} onClick={() => setPage((v) => Math.max(1, v - 1))}>Назад</button>
          <span className="text-slate-400 text-xs md:text-sm">Страница {page} из {totalPages} · прогнозов {total}</span>
          <button className="rounded-md border border-slate-700 px-3 py-1.5 text-slate-300 disabled:opacity-50" disabled={page >= totalPages} onClick={() => setPage((v) => Math.min(totalPages, v + 1))}>Вперёд</button>
        </div>
      )}
    </div>
  );
