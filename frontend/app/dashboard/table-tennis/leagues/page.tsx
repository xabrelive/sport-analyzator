...
import { getTableTennisLeagues, type TableTennisLeaguesPageResponse } from "@/lib/api";

export default function TableTennisLeaguesPage() {
  const [data, setData] = useState<TableTennisLeaguesPageResponse | null>(null);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getTableTennisLeagues(page, pageSize, query)
      .then((res) => {
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <p className="text-slate-400 text-sm mb-4">
        Лиги с быстрым переходом в карточку и пагинацией.
      </p>
      ...
      {(data?.items?.length ?? 0) === 0 ? (
        <p className="text-slate-500">Лиги не найдены.</p>
      ) : (
        <div className="rounded-lg border border-slate-700 overflow-hidden bg-slate-800/40">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {data?.items.map((l) => (
                  <tr key={l.id} className="border-b border-slate-700/60 hover:bg-slate-700/30 transition">
                    <td className="px-4 py-3">
                      <Link
                        href={`/dashboard/table-tennis/leagues/${encodeURIComponent(l.id)}`}
                        className="text-white hover:text-emerald-300"
                      >
                        {l.name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-center tabular-nums text-slate-300">{l.matches_total}</td>
                    <td className="px-4 py-3 text-center tabular-nums text-slate-300">{l.matches_finished}</td>
                    <td className="px-4 py-3 text-center tabular-nums text-slate-300">{l.matches_upcoming}</td>
                  </tr>
                ))}
              </tbody>
            </table>
