...
async def fetch_line_data(
    ...
    max_odds = max(1, getattr(settings, "betsapi_line_max_odds_requests_per_run", 100))
    ids_for_odds = ids_for_odds[:max_odds]
    if ids_for_odds:
        logger.info(
            "BetsAPI line: requesting odds for %s matches (already have line odds: %s, upcoming total: %s)",
            len(ids_for_odds),
