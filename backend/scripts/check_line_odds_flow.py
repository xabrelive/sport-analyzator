...
async def main():
    ...
    print()
    print("=" * 60)
    print("2) Запрос event/odds для первых 3 upcoming (как в пайплайне линии)")
    print("=" * 60)
    ids_for_odds = upcoming_ids[:3]
    events_with_lists = [e for e in events if isinstance(e, dict) and e.get("id") is not None]
    events2 = await collector.fetch(
        ...
        events_from_lists=events_with_lists,
        event_ids_for_view=[],
