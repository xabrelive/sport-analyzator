...
## 3. GET /v2/event/odds?event_id=id

**Что приходит:**

- `results.stats` — служебное (matching_dir, odds_update по рынкам).
- `results.odds` — объект **рынок → список снимков**:
  - Ключи для НТ: **92_1**, **92_2**, **92_3** (92_1 = Match Winner 2-Way, 92_2 = Asian Handicap, 92_3 = Over/Under). Дополнительные рынки (92_4, 92_5, … до 92_25) зависят от тарифа BetsAPI — мы запрашиваем `odds_market=1,2,...,25` (тоталы/форы по сетам 1–7, инд. тоталы и т.д.).
  - Значение — список элементов: `id`, `home_od`, `away_od`, `ss`, `add_time` (unix), при наличии — `handicap`, `total`, `line` (линия гандикапа/тотала). Допускаются также поля `home_odd`/`away_odd` вместо `home_od`/`away_od`. Линия может приходить под ключами `handicap`, `total`, `line`, `handicap_line`, `total_line`, `spread`, `point`, `line_value`; парсер пробует их по порядку и нормализует строки (запятая → точка, unicode минус → минус).

**Что мы делаем в коллекторе:** по каждому рынку проходим **все снимки** из списка. Для каждого снимка берём `home_od`, `away_od`, `add_time`, линию из `handicap` / `total` / `line`. Для **92_3** (Over/Under) исходы маппим в **"over"** / **"under"**; для 92_1 и 92_2 — **"home"** / **"away"**. Передаём в нормалайзер массив снимков с `snapshot_time` и `line_value`.

**Что пишем в БД:**

- **Все снимки** по каждому рынку → отдельная запись `OddsSnapshot` с `snapshot_time` = `add_time` из API (история котировок).
- `OddsSnapshot`: `match_id`, `bookmaker`="b365", `market`="92_1"|"92_2"|"92_3", `selection`="home"|"away"|"over"|"under", `odds`, `implied_probability`, **`line_value`** (для 92_2 гандикап, для 92_3 тотал), **`snapshot_time`** (время снимка из API), **`score_at_snapshot`** (счёт в момент котировки из `ss` в снимке), `phase`=line|live.
- **Match.odds_stats** — копия `results.stats` из ответа GET /v2/event/odds (обновляется при каждом запросе коэффициентов по матчу).

**Итог по коэффициентам:**

- **Сохраняем:** все снимки по времени; три рынка (92_1, 92_2, 92_3); для 92_3 — исходы "over"/"under"; линию гандикапа/тотала в `line_value` при наличии в API; `snapshot_time`; **`results.stats`** в `Match.odds_stats`; **`ss`** из каждого снимка в `OddsSnapshot.score_at_snapshot` (для привязки коэффициентов к счёту в анализе).
- **Не сохраняем:** остальное из ответа odds не требуется.

**Застрявшие в линии:** матчи в статусе SCHEDULED, у которых время начала прошло больше 5 минут и они не пришли в inplay, помечаются как **CANCELLED** (убираются из линии). Если такой матч потом придёт в лайве (тот же состав, тот же порядок хозяин/гость), нормалайзер найдёт его по игрокам и времени и переведёт в **live**.

**Линия без коэффициентов:** если событие из `upcoming` пришло без line-коэффициентов, матч получает статус **`pending_odds`** и не попадает в `/matches/upcoming`. После появления коэффициентов (или если они уже были в БД) статус становится **`scheduled`**. Для настольного тенниса BetsAPI в v2/event/odds часто отдаёт **пустой** `results.odds` по прематчу — тогда кф появятся только когда API начнёт их отдавать (ближе к старту или в лайве). Нужен **ENABLE_SCHEDULED_COLLECTORS=true**, чтобы задача линии запускалась по расписанию и подхватывала кф, как только они появятся в API.

**Связка линия → лайв при разном id:** если в лайве/завершённых приходит тот же матч с другим `event_id`, чем в линии, нормалайзер ищет матч по паре игроков **в том же порядке (хозяин/гость)** и времени начала (окно ±1 ч), в т.ч. среди CANCELLED. Если те же два игрока поменялись местами — это другой матч. Найденный матч привязывается к новому id и обновляется (лайв/счёт/результат). Скрипт проверки дубликатов в БД: `PYTHONPATH=backend python backend/scripts/check_duplicate_matches_by_players.py`.

**Что это за показатели:**
- **`results.stats`** — служебный блок в ответе `GET /v2/event/odds`: метаданные по котировкам (matching_dir, odds_update и т.д.). Сохраняем в **Match.odds_stats** при каждом обновлении коэффициентов по матчу.
- **`ss` внутри снимка** — в каждом элементе списка снимков по рынку API передаёт счёт матча **в момент этой котировки**. Сохраняем в **OddsSnapshot.score_at_snapshot** для аналитики (сопоставление коэффициентов со счётом).

---

## Онлайн последовательность (line/live/odds)

**Важно:** задача линии (`fetch-betsapi-line`) запускается по расписанию **только при `ENABLE_SCHEDULED_COLLECTORS=true`** в `.env`. Если флаг выключен, коэффициенты по линии **не запрашиваются** — в разделе «Линия» матчи будут без кф. Запросы к BetsAPI делаются корректно (GET /v2/event/odds по матчам без line-кф в БД); проблема только в том, что при выключенном расписании задача не выполняется.
...
1. **Линия (`mode=line`)** — интервал `prematch_poll_interval_seconds` (по умолчанию 300 сек):
   ...
   - `GET /v2/event/odds` **только** по матчам, у которых в БД ещё нет line-коэффициентов (лимит `betsapi_line_max_odds_requests_per_run`)
...
2. **Лайв (`mode=live`)** — интервал `live_poll_interval_seconds` (по умолчанию 8 сек):
   ...
   - `GET /v2/event/odds` **только один раз** на старте матча (по матчам, у которых ещё нет `live_odds_fixed_at`); после записи кф выставляется `Match.live_odds_fixed_at`, дальше кф не запрашиваем
...
| **event/odds**| results.odds: 92_1, 92_2, 92_3 (списки снимков); results.stats | Все снимки → OddsSnapshot (market, selection over/under для 92_3, line_value, snapshot_time, **score_at_snapshot** из ss, phase); results.stats → Match.odds_stats | — |
...
На сайте bet365 для НТ есть те же рынки, что отдаёт BetsAPI (источник котировок — bet365). По умолчанию приходят **92_1**, **92_2**, **92_3**. Мы запрашиваем также рынки 4–25 (`odds_market=1,2,...,25`), чтобы при наличии в тарифе получать тоталы и форы по 3–7-му сетам.
...
curl -X POST http://localhost:12000/api/v1/matches/load-history \
...
curl "http://localhost:12000/api/v1/matches/load-history/status?day_from=20160901&day_to=20251231"
...
curl -X POST http://localhost:12000/api/v1/matches/load-history/reset-progress \
  -H "Content-Type: application/json" \
