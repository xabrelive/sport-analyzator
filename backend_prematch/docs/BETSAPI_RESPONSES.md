...
**Важно для линии (прематч):** для событий из `upcoming` BetsAPI часто возвращает **results.odds = {}** (пустой объект) — прематч-коэффициенты по настольному теннису в v2 не всегда доступны. В этом случае коэффициенты появятся в разделе «Линия» только когда API начнёт их отдавать (часто ближе к старту или после перехода матча в лайв; в лайве мы сохраняем кф один раз на старте). Для появления кф нужен включённый сбор по расписанию: **ENABLE_SCHEDULED_COLLECTORS=true**.
