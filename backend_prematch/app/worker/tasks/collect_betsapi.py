...
from app.recommendations.engine import precompute_match_recommendations_async
...
async def _run_recommendations_backfill_async(
    ...
    try:
        ...
        created = await precompute_match_recommendations_async(session_maker, missing)
...
async def _run_result_backfill_async(
    ...
    engine, session_maker = create_worker_engine_and_session()
    try:
        # 1) Результат по итоговому счёту из MatchScore — без внешних запросов.
        extra_from_scores = await _backfill_results_from_scores_from_db(session_maker, limit=None)
        # 2) Сброс попыток у зависших (старше 7 дней без результата).
        ...
        # Матчи без результата, по которым пора сделать очередную попытку
        due_condition = or_(
            and_(
                Match.result_fetch_attempts == 0,
                # Матч уже должен был начаться — можно пробовать добрать результат,
                # не дожидаясь 2 часов.
                Match.start_time <= now,
            ),
            and_(
                Match.result_fetch_attempts == 1,
                Match.start_time <= cutoff_7h,
            ),
            and_(
                Match.result_fetch_attempts == 2,
                Match.start_time <= cutoff_24h,
            ),
        )
        q = (
            select(Match)
...
async def _backfill_results_from_scores_from_db(session_maker, limit: int | None = None) -> int:
    ...
    async with session_maker() as session:
        ...
        norm = Normalizer(session)
        await norm._create_result_from_scores_for_matches([r[0] for r in rows])
        await session.commit()
        logger.info("Result backfill from scores: created results for %s match(es)", len(rows))
        return len(rows)


@celery_app.task(bind=True, name="app.worker.tasks.collect_betsapi.backfill_missing_results")
def backfill_missing_results_task(self):
    """Догрузка результатов по матчам без результата: старт > 2ч назад, не более 3 попыток (2ч, 7ч, 24ч от начала). Запуск по расписанию раз в 1–2 мин."""
    if not settings.betsapi_token:
